...
async function verify() {
  ...
  const result = await Result.tryPromise({
    try: () => $fetch<VerifyResponse>(`/api/invitations/room/${token.value}`),
      ...
      e?.data?.statusMessage || e?.message || 'Invalid or expired invitation',
  });
  loading.value = false;
  result.match({
    ok: (res) => {
...
<template>
  <div class="min-h-screen flex flex-col bg-background">
    ...
    <!-- Error -->
    <div
      v-else-if="errorMessage || !verified"
    ...
    <template v-else>
      <!-- Top bar -->
      <header
        class="border-b px-4 h-14 flex items-center justify-between shrink-0"
      >
        <div class="flex items-center gap-4">
          <h1 class="font-semibold text-sm">Two Sum</h1>
          <span
            class="text-xs rounded-md bg-amber-100 text-amber-800 px-2 py-0.5 font-medium"
          >
            Medium
          </span>
        </div>
...
Explanation: nums[0] + nums[1] == 9</pre>
        ...
        <section class="flex flex-col min-h-0">
          <!-- Editor toolbar -->
          <div
            class="h-10 border-b flex items-center justify-between px-3 shrink-0"
          >
            <select
              v-model="language"
          ...
          <div class="border-t shrink-0 h-40 flex flex-col">
            <div
              class="h-8 px-3 flex items-center gap-1.5 text-xs font-medium text-muted-foreground border-b"
            >
              <IconTerminal2 class="size-3.5" />
              Output
            </div>
            <pre
              class="flex-1 overflow-auto p-3 text-xs font-mono text-muted-foreground whitespace-pre-wrap"
